...
#isolate time slot
intra['t_start'] = intra.init_date + pd.Timedelta(hours=9, minutes=30)
intra['t_end'] = intra.init_date + pd.Timedelta(hours=11)
...
sample['n_entries'] = np.floor((sample.rise - entry_level) / 0.05) + 1
...
def avg_cost_pc(n, e, i):
    num = sum([(e+i*j)*2**j for j in range(n)])
    denom = sum([2**j for j in range(n)])
    return (1 + num/denom)
    
sample['avg_cost_pc'] = sample.apply(
        lambda x: avg_cost_pc(x.n_entries, entry_level, increment),
        axis=1,
)
...
t_exit = intra.init_date + pd.Timedelta(hours=11)
...
sample['weight'] = 2**(sample.n_entries-1)

#how to systematically limit entries/ add stop losses?
