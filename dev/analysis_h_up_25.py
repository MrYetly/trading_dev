...
master = pd.read_csv('../data/high up 25 01222021 agg -253 0.csv')
...
earliest = datetime.strptime('2020-11-10', '%Y-%m-%d')
latest = datetime.strptime('2021-01-21', '%Y-%m-%d')
...
#    trade['pre_vol'] = pre_vol
    ...
    if hist_ts.loc[hist_ts.delta == -1].shape[0] == 0:
        continue
    
    close = hist_ts.loc[hist_ts.delta == -1].close.iloc[0]
    low = row.low
...
#remove innaccuracies from sample
