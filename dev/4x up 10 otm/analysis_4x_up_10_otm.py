...
#find volume on lag of month in past
for m,lag in m_dic.items():
    vol = master.loc[master.delta == lag][['ticker', 'init_date', 'volume']]
    vol = vol.rename(columns = {'volume': f'vol_{m}m'})
    sample = sample.merge(vol, how='inner', on=['init_date','ticker'], validate='1:1')

months = [int(i)-1 for i in list(m_dic.keys())]
...
#drop dq data and strange returns data
...
#restrict to first of each ticker
'''
Note: taking first might be baised at beginning of sample, perhaps first isn't really first
'''
first = sample.groupby('ticker', as_index=False)['init_date'].min()
first['first_check'] = True
sample = sample.merge(first, how='left', on=['init_date', 'ticker'])

#misc. restrict sample
sample = sample.loc[
    (sample.return_o0m < .3)
    & (sample.return_o1m < 1)
    & (sample.return_o2m < 1)
    & (sample.return_o3m < 1)
    & (sample.first_check == True)
...
#print(check_corr.corr()[['return_5', 'return_10', 'return_15', 'return_20']])

#print(sample.return_20.describe())
...
model = OLS(y, exog=X, hasconst=True)
res = model.fit()
res_output = summary_col(res)
print(res_output)
