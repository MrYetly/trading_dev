...
master = pd.read_csv('../data/gap d 25 12262020 agg 5.csv')
print('uncleaned shape:', master.shape)
master = master.loc[(master['ticker'] != 'GLBS') & (master['ticker'] != 'MYT')]
master = master.loc[master['1. open'] > 0.1]

master['constant'] = 1

entry = master.loc[master['delta']== 0]
_exit = master.loc[master['delta'] == 5].copy()
_exit.index = entry.index
master['return'] = (entry['1. open'] - _exit['4. close'])/entry['1. open']
